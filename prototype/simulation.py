...
class Simulation:
    ...
    def cfar(self, X, max_unamb_range, max_unamb_vel, nfft_doppler, nfft_range, signal):
        # 0 noise for now
        noise = 0  # torch.normal(0, 0.0, image.shape)
        ...
        inner_height = k_height // 2
        # r = 2
        # kernel[0, 0, k_height//2 - r:k_height//2 + r, k_width//2 -r:k_width//2 + r] = 0.0
        kernel[0, 0, (k_height - inner_height) // 2: (k_height + inner_height) // 2,
        (k_width - inner_width) // 2: (k_width + inner_width) // 2] = 0
        kernel = kernel / kernel.sum()
        convd = torch.nn.functional.conv2d(input=image, weight=kernel, padding='same', stride=1)
        # how set threshold oof?
        threshold = image > convd * 5
        final_x = threshold.reshape(nfft_doppler, nfft_range).detach().numpy()
        ...
        db = DBSCAN(eps=100, min_samples=20).fit(final_x)
        labels = db.labels_
        ...
        for k in unique_labels:
            ...
            # here number of points is arbitrary
            #     if(xy.shape[0] < 10):
            #         continue
            ...
            if not math.isnan(x.item()) and not math.isnan(y.item()):
                xs.append(x.item())
                ys.append(y.item())
