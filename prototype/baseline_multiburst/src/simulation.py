...
from utils import SNR, resolve_range_ambiguity
...
class Simulation:
    def __init__(self, ranges: [Tensor], velocities: [Tensor], amplitudes: [Tensor]):
        ...
        self.max_unamb_range = []
        self.max_unamb_velocity = []
    ...
    def detect(self, parameters):
        ...
        for i in range(len(pulse_durations)):
            ...
            pulse_duration = param_dict["pulse_duration"][pulse_durations[i]]
            n_pulses = n_pulseses[i]
            pri = param_dict["PRI"][pris[i]]
            ...
            self.max_unamb_range.append(max_unamb_range)
            self.max_unamb_velocity.append(max_unamb_vel)
            ...
            if detections.size != 0:
                detected = True
                apparent_ranges.append(np.fmod(detections[0][0][0], max_unamb_range))
                apparent_velocities.append(np.fmod(detections[0][0][1], max_unamb_vel))

        if detected:
            target = resolve_range_ambiguity(4, apparent_ranges, apparent_velocities, self.max_unamb_range,
                                             self.max_unamb_velocity)
            return target.detach().numpy()
        else:
            return []
    ...
    def get_max_unambigous(self):
        return np.max(self.max_unamb_range), np.max(self.max_unamb_velocity)
