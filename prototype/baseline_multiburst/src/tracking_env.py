...
class TrackingEnv(MultiAgentEnv):
    ...
    def _get_obs(self):
        ...
        # obs[two]['v_wind'] = np.array([self.sim.getWindSpeed()], dtype=np.float32)
        # obs[two]['alt'] = np.array([self.altitude], dtype=np.float32)
    ...
    def reward(self, pds, action_dict):
        ...
        sigma = 0.25
        reward_time = math.exp(
            -(ratio - 1) ** 2 / (2 * sigma ** 2)) if self.episode_reward > self.timestep_limit // 2 else 0  # Gaussian function

        rewards = {}
        rewards[self.agent_ids[0]] = reward_time + reward_pd
        return rewards
