...
def preprocess_observations(obs, opponent_obs, original_obs_space):
    ...
    # n_bursts_original = mask_original.sum()
    mask_opponent = original_opponent_obs['mask']
    # n_bursts_opponent = mask_opponent.sum()  #
...
def build_graphs_from_batch(bursts, batched_mask):
    ...
    for i in range(batch_size):
        ...
        for i, node1 in enumerate(graph.nodes()):
            for j, node2 in enumerate(graph.nodes()):
                if node1 != node2 and 0.5 <= disim_matrix[i, j] and mask[j] * mask[i] == 1:
